...
class ChannelFuseBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, num_res=1, stride=1, downsample=None):
        ...
        self.conv_up = conv3x3(1, planes, stride)
        self.epsilon = 1e-4
...
class Unet_decoder(nn.Module):
    def __init__(self):
        ...
        self.conv1 = res_conv(in_channels=64,out_channels=64)
        self.conv2 = res_conv(in_channels=128,out_channels=128)
        self.conv3 = res_conv(in_channels=128,out_channels=128)
        self.deconv2 = deconv2d_bn(in_channels=128,out_channels=32)
        self.deconv3 = deconv2d_bn(in_channels=128,out_channels=64)
        self.endconv = nn.Conv2d(in_channels=64,out_channels=1,kernel_size=1,stride=1)
    def forward(self,x):
        out1 = self.deconv3(self.conv3(x[2]))

        out1_concat = torch.cat([out1,x[1]],dim=1)
        #128x240x320
        # print(out1_concat.shape)
        out2 = self.deconv2(self.conv2(out1_concat))
        #32x480x640
        out2_concat= torch.cat([out2,x[0]],dim=1)
        #64x480x640
        out = self.endconv(out2_concat)
        return out



class DCFNet(nn.Module):
    def __init__(self, init_channel,out_channel):
        ...
        # stem net
        channel = 32
        self.conv1 = nn.Conv2d(init_channel, channel, kernel_size=3, stride=1, padding=1,
                               bias=False)
        ...
        ################################################################################################
        # stage 1
        self.layer1 = self._make_layer(block=Bottleneck, inplanes=channel, planes=self.channels_list[0],
        ...
        self.last_layer = nn.Sequential(
            ...
            nn.ReLU(inplace=False),
            nn.Conv2d(in_channels=last_inp_channels, out_channels=out_channel,
                      kernel_size=FINAL_CONV_KERNEL, stride=1, padding=1 if FINAL_CONV_KERNEL == 3 else 0)
        )
        self.Unet = Unet_decoder()
...
# my = torch.randn(1,1,240,320)
# sy = torch.randn(1,1,120,160)
# model = DCFNet(init_channel=1,out_channel=1)
# # print(model)
# print(model(x,my,sy).shape)
# if __name__ == '__main__':
#     from torchsummary import summary
#
#     model = DCFNet(spectral_num, mode="C")
#     summary(model, [(8,64,64),(1,64,64),(8,32,32),(8,16,16)], device='cpu')
# if __name__ == '__main__':
#     from torchsummary import summary
#
#     model = DCFNet(spectral_num, mode="C")
#     summary(model, [(3,480,640),(1,480,640),(3,240,320),(3,120,160)], device='cpu')
